...
/*   Updated: 2023/01/10 20:10:31 by aaouassa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_minilbx(t_var *s_var, char **argv)
{
	...
	s_var->addr = mlx_get_data_addr(s_var->img, &s_var->bits_per_pixel,
			&s_var->line_length, &s_var->endian);
...
int	main(int argc, char **argv)
{
	...
	if (argc > 2 || argc == 1)
		printf("you should pass just 2 argument");
	else if (argc == 2)
	{
		if (ft_strcmp(argv[1], "Mandelbrot") == 0)
		{
		...
		mlx_mouse_hook(s_var.mlx_win, mouse_hook, &s_var);
		mlx_loop(s_var.mlx);
	}
}
